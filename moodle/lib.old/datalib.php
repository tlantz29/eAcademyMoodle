...
function get_users($get=true, $search='', $confirmed=false, array $exceptions=null, $sort='firstname ASC',
    ...
    $fullname  = $DB->sql_fullname();
        #----------------sis manager rolewise userlist filter end-------------------------#
      $school_id=get_group_id();
      $count=count($school_id);
//print_r($school_id);exit;
        if($school_id!=false){
             $select = " u.id <> :guestid AND deleted = 0 AND ";
              for($i=0;$i<$count;$i++){
              if($i==0)
                  $select.="( gm.groupid=$school_id[$i] ";
              else
                   $select.=" OR gm.groupid=$school_id[$i] ";

          }
          $select.=" )";
        }
        else
            $select = " u.id <> :guestid AND deleted = 0 ";
        #----------------sis manager rolewise userlist filter end-------------------------#
    $params = array('guestid'=>$CFG->siteguest);

    if (!empty($search)){
        $search = trim($search);
        $select .= " AND (".$DB->sql_like($fullname, ':search1', false)." OR ".$DB->sql_like('u.email', ':search2', false)." OR u.username = :search3)";
    ...
    if ($get) {
        return $DB->get_records_select('{user} u LEFT JOIN {groups_members} gm
                                   ON u.id = gm.userid ', $select, $params, $sort, $fields, $page, $recordsperpage);
    } else {
        return $DB->count_records_select_custom('{user} u LEFT JOIN {groups_members} gm
                                   ON u.id = gm.userid ', $select, $params);
    }
}
...
//   print_r(count($school_id));exit;
     $count=count($school_id);

//     $school_id=1;
        if($school_id!=false){
          $select = "u.deleted <> 1 AND ";
          for($i=0;$i<$count;$i++){
              if($i==0)
                  $select.="( gm.groupid=$school_id[$i] ";
              else
...
//echo "SELECT u.id, u.username, u.email, u.firstname, u.lastname, u.city, u.country,
//                                        u.lastaccess, u.confirmed, u.mnethostid$extrafields
//                                   FROM {user} u LEFT JOIN {groups_members} gm
//                                   ON u.id = gm.userid
//                                   WHERE $select
...
/**
 * @todo Finish documenting this function
 * this function first check the user role is role == sis manager it returns school id
 * @return $school_id on success else false
 */
function get_school_id(){
    ...
    $ras=$DB->get_records_select('role_assignments', $select);  //get multiple record//$select="roleid=$role[0]";
    foreach($ras as $user_role_row ){

        $role_id=$user_role_row->roleid;
    }
        if($role_id!=''){
        $select_id=$select="id=$role_id";
        $role_name_select=$DB->get_records_select('role', $select_id);  //get multiple record//$select="roleid=$role[0]";
        foreach($role_name_select as $role_name_row ){

            $user_role=$role_name_row->name;
        }

            if($user_role=='Manager'){
                $select_school="id=$USER->id";
                $user_details=$DB->get_records_select('user', $select_school);  //get multiple record//$select="roleid=$role[0]";
                foreach($user_details as $user_details_row ){
                    $school_id=$user_details_row->sis_school_id;
                }
                 return $school_id;
            }
            else
    ...
    foreach($ras as $user_role_row ){
      ...
      $role_id=$user_role_row->roleid; 
        ...
        foreach($role_name_select as $role_name_row ){
            ...
            if($role_name_row->name){
                $select_gruop="userid=$USER->id";
                $user_details=$DB->get_records_select('groups_members', $select_gruop);  //get multiple record//$select="roleid=$role[0]";
                foreach($user_details as $user_details_row ){
                   $school_id[]=$user_details_row->groupid; //exit;
                }
//                foreach($school_id as $key=>$val){
//
//                    $group_id[]=$val;
//
//                }
...
//$sql.=" JOIN  courese_group cg ON (cg.course_id = cc .id AND cg.group_id='3')";
    $rs = $DB->get_recordset_sql($sql, $params); 
    foreach($rs as $cat) {
