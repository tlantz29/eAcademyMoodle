...
function get_users($get=true, $search='', $confirmed=false, array $exceptions=null, $sort='firstname ASC',
    ...
    $fullname  = $DB->sql_fullname();
      ...
      $group_id=get_group_id();
        if($group_id!=false)
             $select = " u.id <> :guestid AND deleted = 0 AND gm.groupid=$group_id ";
        else
...
function get_users_listing($sort='lastaccess', $dir='ASC', $page=0, $recordsperpage=0,
  ...
  #----------------sis manager rolewise userlist filter end-------------------------#
   ...
   echo  $school_id=get_group_id();
        if($school_id!=false)
          $select = "u.deleted <> 1 AND gm.groupid=$school_id";
         else
        $select = "u.deleted <> 1 ";
  #----------------sis manager rolewise userlist filter end-------------------------#
    $params = array();
...
#---------------------------------------------------sis customisation ---------------------------#
   ...
   //get user group_id
    $sql="SELECT g.groupid FROM {$CFG->prefix}groups_members g WHERE g.userid ='".$USER->id."' ";
