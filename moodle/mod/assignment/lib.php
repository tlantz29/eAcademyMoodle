    ...
    function submittedlink($allgroups=false) {
        ...
        if (has_capability('mod/assignment:grade', $context)) {
			...
			//// megill
                ...
                $ungraded = $this->count_real_ungraded_submissions($group);

 
					if ($ungraded == '0')
					{ 
					$submitted = '<a href="submissions.php?id='.$this->cm->id.'">'.
						get_string('viewsubmissions', 'assignment', $count).'<br />('.get_string('submissionsnotgraded', 'assignment', $ungraded).')</a>';
					} 
					else
					{
					$submitted = '<a href="submissions.php?id='.$this->cm->id.'">'.
                        get_string('viewsubmissions', 'assignment', $count).'<br /><font color=red>('.get_string('submissionsnotgraded', 'assignment', $ungraded).')</font></a>';
					}
    //// end megill
                //$submitted = '<a href="'.$urlbase.'submissions.php?id='.$this->cm->id.'">'.
...
//// megill
    ...
    function count_real_ungraded_submissions($groupid=0) {
        ...
        return assignment_count_real_ungraded_submissions($this->cm, $groupid, 1);
        ...
        switch ($mode) {
            case 'grade':                         // We are in a main window grading
                if ($submission = $this->process_feedback()) {
                  /***************************************************************************************************/
//                    $sub[]=$submission;
//                    require_once('../../xmlrpc/assignment_grade.php');
//                     //assignment_grade($sub);
//                     $assignmentid=$this->assignment->id;
//                     assignment_withgrade($assignmentid,$sub);
                 /**********************************************************************************************/   
            ...
            case 'fastgrade':
                ...
                foreach ($_POST[$col] as $id => $unusedvalue){
                    ...
                    if ($commenting) {
                        ...
                        $submission->submissioncomment = htmlspecialchars_decode($commentvalue);
                    } else {
...
function assignment_count_real_ungraded_submissions($cm, $groupid=0) {
    global $CFG, $DB;
