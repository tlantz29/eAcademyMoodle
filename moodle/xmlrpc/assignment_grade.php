...
//            $user=get_record_select('user', $user4);
            if($assignment_details->modulename=='quiz' && $assignment_details->timeclose  && $course_id && $course_period_id)
               {
            ...
            if($assignment_details->modulename=='assignment' && $assignment_details->grade>0 && $assignment_details->timedue  && $course_id && $course_period_id)
               {
...
//                            print_r($query_info);exit;
            ...
            if($assignment_details->modulename=='lesson' && $assignment_details->deadline && $assignment_details->practice==0  && $course_id && $course_period_id)
               {
			 ...
			 if($assignment_details->modulename=='forum' && $assignment_details->scale>0 && $assignment_details->assesstimefinish && $course_id && $course_period_id)
               {
                            $query_info=array('assigned_date'=>$assignment_details->assesstimestart,'due_date'=>$assignment_details->assesstimefinish,'course_period_id'=>$course_period_id,'title'=>$assignment_details->name,'points'=>$assignment_details->scale,'course_id'=>$course_id,'moodle_assignment_id'=>$assignment_details->instance,'module'=>forum,'description'=>"");
               }
                ...
                if($assignment_details->modulename=='quiz'  && $course_id && $course_period_id)
                   {
                                ...
                                $query=array('assigned_date'=>$assignment_details->timeopen,'due_date'=>$assignment_details->timeclose,'course_period_id'=>$course_period_id,'title'=>mysql_real_escape_string($assignment_details->name),'points'=>$assignment_details->grade,'description'=>$assignment_details->introeditor[text]);
                ...
                if($assignment_details->modulename=='assignment'  && $course_id && $course_period_id)
                   {
                                ...
                                $query=array('assigned_date'=>$assignment_details->timeavailable,'due_date'=>$assignment_details->timedue,'course_period_id'=>$course_period_id,'title'=>mysql_real_escape_string($assignment_details->name),'points'=>$assignment_details->grade,'description'=>$assignment_details->introeditor[text]);
                                $query_info=array('user'=>$user,'query'=>$query);
                   }
				if($assignment_details->modulename=='forum'  && $course_id && $course_period_id)
                   {
                                $user=array('course_id'=>$course_id,'moodle_assignment_id'=>$assignment_details->instance,'module'=>forum)    ;
                                $query=array('assigned_date'=>$assignment_details->assesstimestart,'due_date'=>$assignment_details->assesstimefinish,'course_period_id'=>$course_period_id,'title'=>mysql_real_escape_string($assignment_details->name),'points'=>$assignment_details->scale,'description'=>$assignment_details->introeditor[text]);
                                $query_info=array('user'=>$user,'query'=>$query);
                   }
                if($assignment_details->modulename=='lesson'  && $assignment_details->practice==0 && $course_id && $course_period_id)
                   {
                                ...
                                $query=array('assigned_date'=>$assignment_details->available,'due_date'=>$assignment_details->deadline,'course_period_id'=>$course_period_id,'title'=>mysql_real_escape_string($assignment_details->name),'points'=>$assignment_details->grade,'description'=>"");
                                $query_info=array('user'=>$user,'query'=>$query);
...
//                $student       =   $DB->get_record_select('user', $select);
                ...
                $recd          =   $DB->get_record_select('grade_items', $selectrec);
                 ...
                 $recd2          =   $DB->get_record_select('grade_grades',$selectrec);
                ...
                if($recd->itemmodule=='lesson' && $rslt->finalgrade){
				 ...
				 if($recd->itemmodule=='forum' && $rslt->finalgrade){
                    $query=array('student_name'=>$student->username,'districtid'=>$student->districtid,'assignment_id'=>$recd->iteminstance,'course_period_id'=>$course_period_id,'points'=>$rslt->finalgrade);
                    $query_info[]=array('mod'=>forum,'querry'=>$query);
                }
