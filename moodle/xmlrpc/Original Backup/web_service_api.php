...
function subjectupdate($query=0)
{
    ...
    $data->sis_courseid=$query['sisid'];
    $sql="SELECT cc.id FROM {$CFG->prefix}course_categories cc WHERE cc.sis_courseid = $data->sis_courseid AND cc.parent";
...
function courseupdate($query=0)
{
     ...
     if($data->name || $data->description)
      {
          $data->sis_courseid=$query['sisid'];
          $sql="SELECT cc.id FROM {$CFG->prefix}course_categories cc WHERE cc.sis_courseid = $data->sis_courseid";
          $data->id = $DB->get_field_sql($sql);
          $new= $DB->update_record('course_categories', $data);
        }
