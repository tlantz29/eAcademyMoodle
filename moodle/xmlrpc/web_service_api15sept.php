...
function course($query=array())
{
    ...
    $data->sortorder = 999;
    $data->timemodified=time();
     $sql = "SELECT cc.id FROM {$CFG->prefix}course_categories cc WHERE cc.sis_courseid = $data1->parent1 and cc.parent  = 0";
    ...
    foreach($query['username'] as $key=> $value)
        {
            ...
            if(($data1->userid)!="")
                    {
                         enrol_try_internal_enrol($course->id, $data1->userid, $CFG->creatornewroleid,$timestart = $data->enrolstartdate, $timeend = $data->enrolenddate);
                    }
         }
...
function subjectupdate($query=0)
{
    ...
    $data->sis_courseid=$query['sisid'];
    $sql="SELECT cc.id FROM {$CFG->prefix}course_categories cc WHERE cc.sis_courseid = $data->sis_courseid AND cc.parent=0";
...
function courseupdate($query=0)
{
     ...
     if($data->name || $data->description)
      {
          $data->sis_courseid=$query['sisid'];
          $sql="SELECT cc.id FROM {$CFG->prefix}course_categories cc WHERE cc.sis_courseid = $data->sis_courseid AND parent<>0";
          $data->id = $DB->get_field_sql($sql);
          $new= $DB->update_record('course_categories', $data);
        }
    ...
    foreach($query['oldusername'] as $uname=>$uval)
        {
            ...
            else
            {
                ...
                {
                        $role= enrol_try_internal_enrol($data->id, $data2->userid, $CFG->creatornewroleid,$timestart = $data->enrolstartdate, $timeend = $data->enrolenddate);
                }
            }
            $count++;
    }
...
function fetchuser($query=0)
{
   ...
   $sql="SELECT u.id FROM {$CFG->prefix}user u WHERE u.username = '$data->username' AND u.deleted!=1 AND u.auth='mnet' AND u.mnethostid='".$mnethost."'";
   $data->id = $DB->get_field_sql($sql);
...
function add_subgroup_group($query=array()){
   ...
   $data->timecreated  =time();
   $data->courseid     =1;
   $data->sis_school_id     =$query['district_id'];
...
function update_subgroup_group($query=array()){
   ...
   $sql="SELECT u.id FROM {$CFG->prefix}groups u WHERE u.sis_school_id='".$query['district_id']."'";
   $data->id=$DB->get_field_sql($sql);
   $uid= $data->id;
  ...
  $success = $DB->update_record('groups', $data);
  XMLRPC_response(XMLRPC_prepare($data->id), KD_XMLRPC_USERAGENT);
