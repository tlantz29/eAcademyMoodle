...
global $USER;
 ...
 //short by group sis_customosation
         //  $sort = ' cg.group_id DESC';
        $categories = get_categories($parent='none', $sort=NULL, $shallow=true);
...
//$group holds the group name to be shown in front page catagory //list for sis customisation
    $group=array();
    ...
    if ($categories = get_child_categories($category->id)) {   // Print all the children recursively
        ...
        foreach ($categories as $cat) {
            ...
            $first = false;
 #------------------------sis customisation for showing group name on top of catagory name--------------------#
            if($cat->parent==0){
               

                   $sql_gr = "SELECT g.name
                          FROM {course_group} c
                          JOIN {groups} g on g.id = c.group_id
                          WHERE c.course_id=$cat->id
                      ";

                $group_name = $DB->get_records_sql($sql_gr, array());
                foreach($group_name as $key=>$val){
//to avoid rint the group name multiple time
...
function print_course($course, $highlightterms = '') {
 ...
 #------customisation for sis -----------#
            //show only the enroled teacher under the course
            $sql_course="SELECT DISTINCT e.courseid
                      FROM {enrol} e
                      JOIN {user_enrolments} ue ON (ue.enrolid = e.id AND ue.userid =$ra->id)
                    where  e.courseid=$course->id";

            $user_courses = $DB->get_records_sql($sql_course, array(), 0, 1);
    #------customisation for sis -----------#
...
#----get user role------------------#
    ...
    //condition for sis cutomisation
    //to protect category view from teacher and student
    elseif($role_id!=5  && $role_id!=3) {
