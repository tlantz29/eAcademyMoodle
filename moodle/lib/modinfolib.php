...
class cm_info extends stdClass  {
    ...
    private function update_user_visible() {
		...
		if($modcontext){  //added this check to fix Default exception handler: Coding error detected, it must be fixed by a programmer: PHP catchable fatal error Debug: Argument 2 passed to has_capability() must be an instance of context, boolean given
			$userid = $this->modinfo->get_user_id();
			$this->uservisible = true;
			if (!$this->visible or !$this->available){
			...
			}elseif (!empty($CFG->enablegroupmembersonly) and !empty($this->groupmembersonly)
