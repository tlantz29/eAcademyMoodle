...
abstract class moodle_database {
    ...
    public function get_records_select_custom($table, $select, array $params=null, $sort='', $fields='*', $limitfrom=0, $limitnum=0) {
        if ($select) {
            $select = "WHERE $select";
        }
        if ($sort) {
     ...
     * 
     */
     public function get_records_select_menu_custom($table, $select, array $params=null, $sort='', $fields='*', $limitfrom=0, $limitnum=0) {
        $menu = array();
        if ($records = $this->get_records_select_custom($table, $select, $params, $sort, $fields, $limitfrom, $limitnum)) {
            foreach ($records as $record) {
                $record = (array)$record;
                $key   = array_shift($record);
                $value = array_shift($record);
...
/**
 *   This is a custom function for sis use only
     * Count the records in a table which match a particular WHERE clause.
     *
     * @param string $table The database table to be checked against.
     * @param string $select A fragment of SQL to be used in a WHERE clause in the SQL call.
    ...
    public function count_records_select_custom($table, $select, array $params=null, $countitem="COUNT('x')") {
        if ($select) {
          $select = "WHERE $select";//exit;
        }
        return $this->count_records_sql("SELECT $countitem FROM " . $table . " $select", $params);
    }
